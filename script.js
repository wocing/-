...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (askAiBtn) {
        askAiBtn.addEventListener('click', function() {
            ...
            if (window.galleryManager && window.aiChatAssistant) {
                // 从图片URL或标题中找到对应的图片对象
                const currentImage = window.galleryManager.images.find(img => 
                    img.image_url === modalImg.src || 
                    img.title === caption.textContent.split(' - ')[0]
                );
                
                if (currentImage) {
                    window.aiChatAssistant.askAboutImage(currentImage);
                } else {
                    // 如果找不到具体图片，直接打开聊天窗口
                    window.aiChatAssistant.toggleChat();
                }
            }
        });
    }
